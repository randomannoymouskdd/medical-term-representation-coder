...
from model import UMLSPretrainedModel, UMLSPretrainedModel_noneg
...
def train(args, model, train_dataloader, umls_dataset):
    ...
    while True:
        ...
        for _, batch in enumerate(epoch_iterator):
            ...
            if args.use_re and not args.use_rel:
                re_label = batch[9].to(args.device)
            if args.use_rel and not args.use_re:
                re_label = batch[10].to(args.device)
            if args.use_re and args.use_rel:
                re_label = batch[11].to(args.device)
...
def run(args):
    ...
    umls_dataset = UMLSDataset(
        ...
        negative_sampling=args.additional_negative_sampling, debug=False)
    ...
    if args.use_rel and args.use_re:
        rel_label_count = len(umls_dataset.re_rel2id)
    if not args.use_re and not args.use_rel:
        print("Should specific at least one \"use_re\" or \"use_rel\"")
        import sys
        sys.exit()
        ...
        if len(save_list) > 0:
            ...
            if os.path.exists(os.path.join(args.output_dir, 'last_model.pth')):
                model = torch.load(os.path.join(
                    args.output_dir, 'last_model.pth'), map_location='cpu').to(args.device)
                model_load = True
            else:
                model = torch.load(os.path.join(
                    args.output_dir, f'model_{max(save_list)}.pth'), map_location='cpu').to(args.device)
                model_load = True
    if not model_load:
        if args.additional_negative_sampling:
            print("UMLSPretrainedModel")
            # Only support TransE loss now
            ...
            print("UMLSPretrainedModel_noneg")
            model = UMLSPretrainedModel_noneg(device=args.device, model_name_or_path=args.model_name_or_path,
                                          cui_label_count=len(umls_dataset.cui2id),
                                          rel_label_count=rel_label_count,
                                          sty_label_count=len(umls_dataset.sty2id),
                                          re_weight=args.re_weight,
                                          sty_weight=args.sty_weight,
                                          trans_loss_type=args.trans_loss_type).to(args.device)                
        args.shift = 0
        model_load = True
    ...
    if args.do_train:
        ...
        umls_dataset.tokenizer.save_pretrained(args.output_dir)
...
def main():
    ...
    parser.add_argument("--use_re", default=False, type=bool,
                        help="use re or not.")
    parser.add_argument("--use_rel", default=False, type=bool,
                        help="use rel or not.")
    ...
    parser.add_argument("--trans_loss_type", type=str, default="CosineDistMult_MS",
                        choices=["TransE", "TransE_MS", "CosineDistMult_MS"])
