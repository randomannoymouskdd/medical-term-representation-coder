...
class UMLSDataset(Dataset):
    def __init__(self, umls_folder, model_name_or_path, lang, json_save_path=None, max_lui_per_cui=8, max_length=32, negative_sampling=True, debug=False):
        self.debug = debug
    ...
    def calculate_class_count(self):
        ...
        if self.json_save_path:
            ...
            with open(os.path.join(self.json_save_path, "re_rel2id.json"), "w") as f:
                json.dump(self.re_rel2id, f)            
        ...
        print("RE_REL:", len(self.re_rel2id))
    ...
    def __getitem__(self, index):
        ...
        re_rel = re + "|" + rel
        re_rel_index = self.re_rel2id[re_rel]
        if self.negative_sampling:
            return input_ids_0, input_ids_1, input_ids_2, \
                ...
                [re_index] * use_len, \
                [rel_index] * use_len, \
                [re_rel_index] * use_len

        # For model input convinence
        return input_ids_0, input_ids_1, input_ids_0, \
            [cui0_index] * use_len, [cui1_index] * use_len, [cui0_index] * use_len, \
            [sty0_index] * use_len, [sty1_index] * use_len, [sty0_index] * use_len, \
            [re_index] * use_len, \
            [rel_index] * use_len, \
            [re_rel_index] * use_len, [re_rel_index] * use_len
...
if __name__ == "__main__":
    umls_dataset = UMLSDataset(umls_folder="../umls",
                               ...
                               lang=None, debug=True, negative_sampling=False)
    umls_dataloader = fixed_length_dataloader(umls_dataset, fixed_length=256, num_workers=4)
    now_time = time()
