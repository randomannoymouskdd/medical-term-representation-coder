...
from trans import TransE, TransE_MS, TransE_batch, CosineDistMult_MS
...
class UMLSPretrainedModel_noneg(nn.Module):
    def __init__(self, device, model_name_or_path,
                 cui_label_count, rel_label_count, sty_label_count,
                 re_weight=1.0, sty_weight=0.1,
                 cui_loss_type="ms_loss",
                 trans_loss_type="TransE", trans_margin=1.0):
        super(UMLSPretrainedModel_noneg, self).__init__()
        ...
        if self.trans_loss_type == "TransE":
            self.re_loss_fn = TransE(trans_margin)
            #self.re_loss_fn = TransE_batch(trans_margin)
        if self.trans_loss_type == "TransE_MS":
            self.re_loss_fn = TransE_MS()
        if self.trans_loss_type == "CosineDistMult_MS":
            self.re_loss_fn = CosineDistMult_MS(self.rel_label_count, self.feature_dim)
        self.re_embedding = nn.Embedding(
            self.rel_label_count, self.feature_dim)
    ...
    def forward(self,
                ...
                sty_label_0, sty_label_1, sty_label_2,
                re_label):
        input_ids = torch.cat((input_ids_0, input_ids_1), 0)
        cui_label = torch.cat((cui_label_0, cui_label_1))
        sty_label = torch.cat((sty_label_0, sty_label_1))

        use_len = input_ids_0.shape[0]

        pooled_output = self.get_sentence_feature(
            input_ids)  # (2 * pair) * re_label
        logits_sty = self.linear_sty(pooled_output)
        sty_loss = self.sty_loss_fn(logits_sty, sty_label)
        ...
        re_output = self.re_embedding(re_label)
        if self.trans_loss_type == "TransE":
            re_loss = self.re_loss_fn(cui_0_output, cui_1_output, re_output)
        if self.trans_loss_type == "TransE_MS":
            re_loss = self.re_loss_fn(cui_0_output, cui_1_output, re_output, cui_label_0, cui_label_1)
        if self.trans_loss_type == "CosineDistMult_MS":
            re_loss = self.re_loss_fn(cui_0_output, cui_1_output, re_label, cui_label_0, cui_label_1)
